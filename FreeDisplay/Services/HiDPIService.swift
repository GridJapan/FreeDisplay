...
final class HiDPIService: @unchecked Sendable {
    ...
    private func enableHiDPIPlist(vendor: UInt32, product: UInt32,
        ...
        guard let data = try? PropertyListSerialization.data(fromPropertyList: plist, format: .xml, options: 0) else {
            return "Could not generate the plist data"
        }
        ...
        do {
            ...
            return "Could not write the temporary file: \(error.localizedDescription)"
        }
    ...
    private func executePrivilegedCommand(_ command: String) -> String? {
        ...
        guard let appleScript = NSAppleScript(source: script) else {
            return "Could not create the AppleScript"
        }
        ...
        if let error = error {
            ...
            return "Administrator authorisation failed: \(msg)"
        }
        return nil
    }
